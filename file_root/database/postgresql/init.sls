...
pg_hba_conf:
  file.managed:
    ...
    - contents: |
        local   all             postgres                                ident map=root_as_others
        local   all             all                                     ident map=root_as_others
        host    all             all             127.0.0.1/32            md5
        host    all             all             ::1/128                 md5
        host    all             all             10.0.0.0/8              md5
        host    all             all             {{ salt['openstack_utils.minion_ip'](grains['id']) }}/32     ident  map=root_as_others
    - require: 
{% for pkg in postgresql['packages'] %}
